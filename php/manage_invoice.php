<?php
  ...
  function searchInvoice($text, $column) {
    ...
    if($con) {
      ...
      if($column == 'INVOICE_ID')
        $query = "SELECT * FROM invoices INNER JOIN customers ON invoices.CUSTOMER_ID = customers.NAME WHERE CAST(invoices.$column AS VARCHAR(9)) LIKE '%$text%'";
      else if($column == "INVOICE_DATE")
        $query = "SELECT * FROM invoices INNER JOIN customers ON invoices.CUSTOMER_ID = customers.NAME WHERE invoices.$column = '$text'";
      else
        $query = "SELECT * FROM invoices INNER JOIN customers ON invoices.CUSTOMER_ID = customers.NAME WHERE UPPER(customers.$column) LIKE '%$text%'";
  ...
  function printInvoice($invoice_id) {
    require "db_connection.php";
    if($con) {
      $query = "SELECT * FROM invoices INNER JOIN customers ON invoices.CUSTOMER_ID = customers.NAME WHERE INVOICE_ID = $invoice_id";
      $result = mysqli_query($con, $query);
