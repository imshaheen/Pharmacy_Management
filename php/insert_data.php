...
if (mysqli_query($con, $sql)) {
    ...
    // Update stock quantities for each medicine
    $medicineNames = explode(',', $combinedMedicineNames);
    $issueQuantities = explode(',', $combinedIssueQuantities);

    for ($i = 0; $i < count($medicineNames); $i++) {
        $medicineName = trim($medicineNames[$i]);
        $issueQuantity = trim($issueQuantities[$i]);

        $updateQuery = "UPDATE medicines_stock SET QUANTITY = QUANTITY - $issueQuantity WHERE NAME = '$medicineName'";
        if (!mysqli_query($con, $updateQuery)) {
            echo "Error updating stocks for medicine $medicineName: " . mysqli_error($con);
        }
    }
