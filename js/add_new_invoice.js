...
class MedicineInfo {
  constructor(name, batch_id, expiry_date, quantity) {
    this.name = name;
...
function addInvoice() {
  // save invoice
  var customers_name = document.getElementById('customers_contact_number');
  var customers_contact_number = document.getElementById('customers_name');
  var invoice_number = document.getElementById('invoice_number');
  var invoiceDate = document.getElementById('INVOICE_DATE');
  ...
  var fitForWork = document.getElementById('fit_for_work');
  var cDoctor = document.getElementById('c_doctor');
  ...
  else if(isCustomer(customers_name.value) == "true") {
  ...
  else if(!notNull(invoiceDate.value, "date_error"))
  invoiceDate.focus();

  else if(!notNull(inTime.value, "inTime"))
  inTime.focus();

  else if(!notNull(out_time.value, "out_time"))
  out_time.focus();

  else if(!notNull(Disease.value, "Disease"))
  Disease.focus();

  else if(!notNull(fit_for_work.value, "fit_for_work"))
  fit_for_work.focus();

  else if(!notNull(c_doctor.value, "c_doctor"))
  c_doctor.focus();
    ...
    for(var i = 1; i < row_count; i++) {
      ...
      //   discount.focus();
        ...
        flag = true;
        //alert("row " + i + "perfect...");
        medicines[i-1] = new MedicineInfo(medicine_name.value, batch_id.value, expiry_date.value, quantity.value);
        // , mrp.value, discount.value, total.value
      }
    ...
    for(var i = 0; i < row_count - 1; i++) {
      ...
      addSale(customers_name.value, customers_contact_number.value, invoice_number.value, medicines[i].name, medicines[i].batch_id, medicines[i].expiry_date, medicines[i].quantity);
      // , medicines[i].mrp, medicines[i].discount, medicines[i].total
    }
    addNewInvoice(customers_name.value, customers_contact_number.value, invoice_date.value);
    // , total_amount.value, total_discount.value, net_total.value
...
function addSale(customers_name, customers_contact_number, invoice_number, medicine_name, batch_id, expiry_date, quantity) {
  // , mrp, discount, total
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if(xhttp.readyState = 4 && xhttp.status == 200)
  ...
  xhttp.open("GET", "php/add_new_invoice.php?action=add_sale&customers_name=" + customers_name + "&customers_contact_number=" + customers_contact_number + "&invoice_number=" + invoice_number + "&medicine_name=" + medicine_name + "&batch_id=" + batch_id + "&expiry_date=" + expiry_date +  "&quantity=" + quantity, true);
  // + "&mrp=" + mrp + "&discount=" + discount + "&total=" + total,
  xhttp.send();
}

function addNewInvoice(customers_name, customers_contact_number, invoice_date) {
  // , total_amount, total_discount, net_total
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if(xhttp.readyState = 4 && xhttp.status == 200)
      document.getElementById("invoice_acknowledgement").innerHTML =  xhttp.responseText;
  };
  xhttp.open("GET", "php/add_new_invoice.php?action=add_new_invoice&customers_name=" + customers_name + "&customers_contact_number=" + customers_contact_number + "&invoice_date=" + invoice_date, true);
  // + "&total_amount=" + total_amount + "&total_discount=" + total_discount + "&net_total=" + net_total, 
  xhttp.send();
}
