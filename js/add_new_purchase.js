  ...
  if(!notNull(suppliers_name.value, "supplier_name_error"))
    ...
    for(var i = 1; i < row_count; i++) {
      ...
      // }
        ...
        medicineStockRow[i-1] = new MedicineStock(medicine_name.value, batch_id.value, expiry_date.value, quantity.value);
    ...
    for(var i = 0; i < row_count - 1; i++) {
      ...
      addMedicineStock(medicineStockRow[i].name, medicineStockRow[i].batch_id, medicineStockRow[i].expiry_date, medicineStockRow[i].quantity, invoice_number.value);
    }
...
function addMedicineStock(name, batch_id, expiry_date, quantity, invoice_number) {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if(xhttp.readyState = 4 && xhttp.status == 200)
      xhttp.responseText;
  };
  xhttp.open("GET", "php/add_new_purchase.php?action=add_stock&name=" + name + "&batch_id=" + batch_id + "&expiry_date=" + expiry_date + "&quantity=" + quantity + "&invoice_number=" + invoice_number, false);
  xhttp.send();
}

function addNewPurchase(suppliers_name, invoice_number, payment_type, invoice_date) {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if(xhttp.readyState = 4 && xhttp.status == 200)
      document.getElementById('purchase_acknowledgement').innerHTML = xhttp.responseText;
  };
  xhttp.open("GET", "php/add_new_purchase.php?action=add_new_purchase&suppliers_name=" + suppliers_name + "&invoice_number=" + invoice_number + "&payment_type=" + payment_type + "&invoice_date=" + invoice_date + "&invoice_date=" + invoice_date, true);
  xhttp.send();
}
